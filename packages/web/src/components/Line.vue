...
const line = computed(() => {
  ...
  if (
    !svgSize ||
    !svgOffset ||
    !linearFnVariables.value ||
    (props.points[0]?.[0] === props.points[1]?.[0] &&
      props.points[0]?.[1] === props.points[1]?.[1])
  )
    return;
...
const strokeDasharray = computed(
  () => `${props.pathWidth} ${2 * props.pathWidth}`,
);
</script>

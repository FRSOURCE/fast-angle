...
export function useAnglesLabels(
  ...
  watchThrottled(
    [intersectionPosition, anglesBisectors, angle],
    () => {
      ...
      const acuteX =
        Math.cos(Math.atan(acuteSlope)) * (distance + (90 - acuteAngle) * 0.4) +
        intersectionPosition.value.x;

      const obtuseAngle = 180 - acuteAngle;
      const obtuseX =
        -Math.cos(Math.atan(obtuseSlope)) *
          (distance + (90 - obtuseAngle) * 0.4) +
        intersectionPosition.value.x;

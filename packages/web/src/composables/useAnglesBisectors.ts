...
export function useAnglesBisectors(
  ...
  return computed(() => {
    if (
      !intersectionPosition.value ||
      !line1Variables.value ||
      !line2Variables.value ||
      !angle.value
    )
      return;
    ...
    let slope1 = Math.tan(
      (Math.atan(line1Vars.slope) + Math.atan(line2Vars.slope)) / 2,
    );
    let slope2 = -1 / slope1;
    let intercept1 =
      line1Vars.slope * intersectionPosition.value.x +
      line1Vars.intercept -
      slope1 * intersectionPosition.value.x;
    let intercept2 =
      line1Vars.slope * intersectionPosition.value.x +
      line1Vars.intercept -
      slope2 * intersectionPosition.value.x;

    const bisect1Angle =
      getAngle([
        [
          [0, line1Vars.intercept],
          [
            intersectionPosition.value.x,
            line1Vars.slope * intersectionPosition.value.x +
              line1Vars.intercept,
          ],
        ],
        [
          ...
          [
            intersectionPosition.value.x,
            slope1 * intersectionPosition.value.x + intercept1,
          ],
        ],
      ] as Lines) * 2;

...
const SPECTRUM_401_950 = Array.from(
  { length: 550 },
  (_, i) => (i + 401) / 1000,
);
...
export function useColor(value: Ref<unknown>) {
  return computed(() => {
    ...
    const [hue, saturation, lightness] = new ColorHash({
      lightness: lightnessSpectrum,
    }).hsl(text);

    return `hsl(${[
      hue,
      `${
        (isDark.value && lightness < 0.3) || (!isDark.value && lightness > 0.7)

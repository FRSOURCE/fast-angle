...
export function usePanzoom(
  ...
  const zoom = computed({
    set(v: number) {
      if (v > 10) zoomInternal.value = 10;
      else if (v < 0.1) zoomInternal.value = 0.1;
      else zoomInternal.value = v;
  ...
  const boundingRect = computed(() =>
    unrefElement(elementRef)?.getBoundingClientRect(),
  );

  const mapZoomValue = (zoomValue: number) => {
    zoomValue = (zoomValue + 1800) / 1800;
    if (zoomValue <= 0) return 0.01;
    return zoomValue;
  };
  ...
  const updateActiveState = ({
    ...
    else active.value = nextActiveValue;
  ...
  usePinch(
    ({
      ...
      if (boundingRect.value) {
        ...
        offset.value[0] -=
          (originValue[0] - boundingRect.value.width / 2) *
          (zoom.value - oldZoomValue);
        offset.value[1] -=
          (originValue[1] - boundingRect.value.height / 2) *
          (zoom.value - oldZoomValue);
      }

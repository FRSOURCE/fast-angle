...
export const usePanzoom = (elementRef: MaybeElementRef, options?: MaybeComputedRef<GenericOptions | undefined>) => {
  ...
  const updateActiveState = ({ wheeling, dragging, pinching, intentional, enabled, touches, metaKey, ctrlKey, altKey }: { wheeling: boolean; dragging: boolean; pinching: boolean; intentional: boolean; enabled: boolean; touches: number; metaKey: boolean; ctrlKey: boolean; altKey: boolean }) => {
    const nextActiveValue = !enabled || !intentional
      ? false
      : (dragging && (touches === 2 || metaKey || altKey)) || pinching || (wheeling && !ctrlKey)
  ...
  useDrag(({ wheeling, dragging, pinching, touches, intentional, delta, metaKey, ctrlKey, altKey }) => {
    if (!updateActiveState({ wheeling, dragging, pinching, intentional, enabled: enabled.value, metaKey, ctrlKey, altKey, touches }))
      return
  ...
  useWheel(({ event, delta, wheeling, dragging, pinching, touches, intentional, metaKey, ctrlKey, altKey }) => {
    if (!updateActiveState({ dragging, pinching, wheeling, intentional, enabled: enabled.value, touches, metaKey, ctrlKey, altKey }))
      return
  ...
  usePinch(({ offset: [zoomValue], origin: originValue, pinching, dragging, wheeling, metaKey, ctrlKey, altKey, touches, intentional }) => {
    if (!updateActiveState({ dragging, pinching, wheeling, intentional, enabled: enabled.value, touches, metaKey, altKey, ctrlKey }))
      return
